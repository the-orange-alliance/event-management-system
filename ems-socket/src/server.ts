...
import DriverStationRoom from "./rooms/DriverStation";
...
const driverStationRoom = new DriverStationRoom(socket);
...
socket.on("connection", (client: Socket) => {
  ...
  client.on("identify", (clientStr: string, rooms: string[]) => {
    ...
    if (Array.isArray(rooms)) {
      ...
      for (const room of rooms) {
        ...
        if (room === "ds") {
          driverStationRoom.addClient(client);
        }
      }
    } else {
  ...
  client.on("disconnect", () => {
    ...
    driverStationRoom.removeClient(client);
  });
  client.on("drip", () => {
    client.emit("drop");
  });
