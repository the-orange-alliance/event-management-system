...
class DatabaseManager {
  ...
  public getMatchAndParticipants(tournamentLevel: number, operator?: string): Promise<any[]> {
    return new Promise<any[]>((resolve, reject) => {
      operator = operator || "=";
      const query: string = `SELECT "match".match_key, "match".match_detail_key, "match".match_name, "match".tournament_level, "match".scheduled_time, "match".start_time, "match".prestart_time, "match".field_number, "match".cycle_time, "match".red_score, "match".blue_score, GROUP_CONCAT(match_participant.team_key) AS participants, GROUP_CONCAT(match_participant.match_participant_key) AS participant_keys, GROUP_CONCAT(match_participant.alliance_key) AS alliance_keys, GROUP_CONCAT(match_participant.station) AS stations, GROUP_CONCAT(match_participant.surrogate) AS surrogates FROM "match", "match_participant" WHERE "match".match_key = "match_participant".match_key AND "match".tournament_level${operator}"${tournamentLevel}" GROUP BY "match".match_key;`;
      this._db.all(query, (error: any, rows: any[]) => {
        if (error) {
          reject(error);
        } else {
...
export const RoverRuckusRankingQuery = `SELECT EVERYTHING`;