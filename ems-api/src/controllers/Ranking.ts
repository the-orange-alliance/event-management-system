...
router.get('/', (req: Request, res: Response, next: NextFunction) => {
  DatabaseManager.selectAllOrderBy('ranking', '"ranking".rank')
    .then((rows: any[]) => {
...
router.get('/teams', (req: Request, res: Response, next: NextFunction) => {
  DatabaseManager.selectAllFromJoinOrderBy('ranking', 'team', 'team_key', '"ranking".rank')
    .then((rows: any) => {
      res.send({ payload: rows });
    })
    .catch((error: any) => {
...
router.get('/calculate/:tournament_level', (req: Request, res: Response, next: NextFunction) => {
  if (!req.query.type) {
    ...
    DatabaseManager.getMatchResultsForRankings(req.query.type as string, req.params.tournament_level)
      .then((rows: any[]) => {
        logger.info(`Re-calculating rankings for ${req.query.type}.`);
        console.log(rows);
        const ranker: IMatchRanker = getRankerByType(req.query.type as string);
...
router.post('/', (req: Request, res: Response, next: NextFunction) => {
  ...
  DatabaseManager.insertValues('ranking', req.body.records)
    .then((rows: any[]) => {
      res.send({ payload: rows });
    })
    .catch((error: any) => {
